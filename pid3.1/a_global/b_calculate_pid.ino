void calculate_pid(float current_roll, float stable_roll, float current_pitch,
                   float stable_pitch, float current_yaw, float stable_yaw,
                   float curent_depth, float stable_depth) {
  ...
  pid_output_roll = pid_p_gain_roll * pid_error_temp + pid_i_mem_roll +
                    pid_d_gain_roll * (pid_error_temp - pid_last_error_roll);
  ...
  pid_output_pitch = pid_p_gain_pitch * pid_error_temp + pid_i_mem_pitch +
                     pid_d_gain_pitch * (pid_error_temp - pid_last_error_pitch);
  ...
  pid_i_mem_yaw =
      pid_i_mem_yaw + pid_i_gain_yaw * pid_error_temp; // integral(Ki*error)
  ...
  pid_output_yaw = pid_p_gain_yaw * pid_error_temp + pid_i_mem_yaw +
                   pid_d_gain_yaw * (pid_error_temp - pid_last_error_yaw);
  ...
  pid_output_depth = pid_p_gain_depth * pid_error_temp + pid_i_mem_depth +
                     pid_d_gain_depth * (pid_error_temp - pid_last_error_depth);
